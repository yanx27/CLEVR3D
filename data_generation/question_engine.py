...
def relate_handler(scene_struct, inputs, side_inputs):
  ...
  for idx, obj in enumerate(scene_struct['objects']):
    idx_to_objID[str(idx)] = obj['id']

  objID_to_idx = {}
  for k, v in idx_to_objID.items():
    objID_to_idx[v] = k
  results = scene_struct['relationships'][relation][idx_to_objID[str(inputs[0])]]
  results = [objID_to_idx[str(i)] for i in results]
