...
slots; by convention <Z> = Size, <C> = Color, <M> = Material, <S> = Shape, <L> = Label.
...
parser.add_argument('--input_scene_file',
                    default='./3dssg_scenes.json',
                    help="JSON file containing ground-truth scene information for all scenes")
parser.add_argument('--metadata_file', default='3dssg_metadata.json', help="JSON file containing metadata about functions")
parser.add_argument('--synonyms_json', default='3dssg_synonyms.json', help="JSON file defining synonyms for parameter values")
parser.add_argument('--template_dir', default='templates', help="Directory containing JSON templates for questions")
parser.add_argument('--output_questions_file',
                    default='../output/questions.json',
                    help="The output file to write containing generated questions")
parser.add_argument('--output_questions_only_file',
                    default='../output/questions_only.json',
                    help="The output file to write containing generated questions only")
parser.add_argument('--only_relate',
                    action='store_true',
                    help="Generate questions based on the templates that include relationship",
                    default=False)
parser.add_argument('--more_relate', default='same object type', type=str, help="Generate more relate questions")
parser.add_argument('--only_query',
                    action='store_true',
                    help="Generate questions based on the templates that include query (like query_color)",
                    default=False)
parser.add_argument('--only_query_label',
                    action='store_true',
                    help="Generate questions based on the templates that include query label",
                    default=False)

# Control which and how many images to process
parser.add_argument('--scene_start_idx',
                    default=0,
                    type=int,
...
parser.add_argument('--num_scenes',
                    ...
                    "generates questions for all scenes in the input file starting from " + "--scene_start_idx")
...
parser.add_argument('--templates_per_image',
                    default=100,
                    type=int,
                    help="The number of different templates that should be instantiated " + "on each image")
parser.add_argument('--instances_per_template',
                    default=1,
                    type=int,
                    help="The number of times each template should be instantiated on an image")

# Misc
parser.add_argument('--reset_counts_every',
                    default=250,
                    type=int,
                    ...
                    "will result in longer runtimes.")  # for all_questions_orig_bak, it is 250
parser.add_argument('--verbose', action='store_true', help="Print more verbose output")
parser.add_argument('--time_dfs', action='store_true', help="Time each depth-first search; must be given with --verbose")
parser.add_argument('--profile', action='store_true', help="If given then run inside cProfile")

parser.add_argument('--num_of_attributes', default=4, type=int, help="The number of attributes for each question")
parser.add_argument('--num_of_objects', default=3, type=int, help="The number of attributes for each question")
parser.add_argument('--total_num_of_questions', default=1000, type=int, help="The total number of questions")
...
def precompute_filter_options(scene_struct, metadata):
    ...
    if metadata['dataset'] == '3DSSG':
        attr_keys = ['size', 'color', 'material', 'shape', 'label']
    else:
...
def find_filter_options(args, object_idxs, scene_struct, metadata):
    # Keys are tuples (size, color, shape, material) (where some may be None)
    # and values are lists of object idxs that match the filter criterion
    ...
    for k, vs in scene_struct['_filter_options'].items():
        if k[-1] is None:
            continue
        if k.count(None) >= (4 - args.num_of_attributes):
            tmp = sorted(list(object_idxs & vs))  # intersect
            if tmp:
                attribute_map[k] = sorted(list(object_idxs & vs))
    return attribute_map


def find_relate_filter_options(args, object_idx, scene_struct, metadata, unique=False, include_zero=False, trivial_frac=0.1):
    options = {}
    if '_filter_options' not in scene_struct:
        precompute_filter_options(scene_struct, metadata)
    ...
    for relationship in scene_struct['relationships']:
        if args.only_relate:
            if relationship not in [args.more_relate]:
                continue
        try:
            related = set(scene_struct['relationships'][relationship][idx_to_objID[str(object_idx)]])
            related_new = []
            for i in related:
                related_new.append(int(objID_to_idx[str(i)]))
            related = set(related_new)
        except:
            continue
        for filters, filtered in scene_struct['_filter_options'].items():
            ...
            if intersection:
                if filters.count(None) >= (4 - args.num_of_attributes):
...
def add_empty_filter_options(args, attribute_map, metadata, num_to_add):
    # Add some filtering criterion that do NOT correspond to objects

    if metadata['dataset'] == '3DSSG':
        tmp = ['Size', 'Color', 'Material', 'Shape']
        to_replace = random.sample(tmp, k=4 - args.num_of_attributes)
        for idx, v in enumerate(tmp):
            if v in to_replace:
                tmp[idx] = None
        attr_keys = tmp
        attr_keys.append('Label')
    else:
        assert False, 'Unrecognized dataset'
    ...
    for t in attr_keys:
        if t:
            attr_vals.append(metadata['types'][t])
        else:
...
def other_heuristic(text, param_vals):
  ...
  """
    ...
    if param_vals.keys() != target_keys:
        return text
    key_pairs = [('<Z>', '<Z2>'), ('<C>', '<C2>'), ('<M>', '<M2>'), ('<S>', '<S2>'), ('<L>', '<L2>')]
    remove_other = False
...
def instantiate_templates_dfs(args,
                              scene_struct,
                              template,
                              metadata,
                              answer_counts,
    ...
    while states:
        ...
        if answer is None:
            # Why None?
            # For example, query size, the size is None
            # states contain all the satisfied filter options
            # if the answer is None,
            # then go and choose another one
        ...
        for constraint in template['constraints']:
            if constraint['type'] == 'NEQ':
                ...
                if i is not None and j is not None :
                    ...
                    class_i = scene_struct['objects'][input_i[0]]['rio27']
                    class_j = scene_struct['objects'][input_j[0]]['rio27']
                    # if class_i in exclude_class or class_j in exclude_class:
                    #     skip_state = True
                    #     break
                    if class_i == class_j:
                        if verbose:
        ...
        if state['next_template_node'] == len(template['nodes']):
            ...
            if has_relate:
                degen = qeng.is_degenerate(q, metadata, scene_struct, answer=answer, verbose=verbose)
                if degen:
                    continue
        ...
        next_node = template['nodes'][state['next_template_node']]  # idx 1
        ...
        if next_node['type'] in special_nodes:
            if next_node['type'].startswith('relate_filter'):
                ...
                filter_options = find_relate_filter_options(args,
                                                            answer,
                                                            scene_struct,
                                                            metadata,
                                                            unique=unique,
                ...
                filter_options = find_filter_options(args, answer, scene_struct, metadata)
                ...
                if next_node['type'] == 'filter_unique':
                    ...
                    if next_node['type'] == 'filter_exist':
                        ...
                        add_empty_filter_options(args, filter_options, metadata, num_to_add)
                    elif next_node['type'] == 'filter_count' or next_node['type'] == 'filter':
                        # For filter_count add nulls equal to the number of singletons
                        num_to_add = sum(1 for k, v in filter_options.items() if len(v) == 1)  # 18
            ...
            for k in filter_option_keys:
                ...
                for param_name, param_val in zip(filter_side_inputs, k):
                    ...
                    if param_val is not None:
                        ...
                        if metadata['dataset'] == '3DSSG' and param_type == 'Label':
                            param_val = 'thing'
                        else:
                            param_val = ''
                        cur_next_vals[param_name] = param_val
            ...
            if len(next_node['side_inputs']) != 1:
                import pdb
                pdb.set_trace()
    ...
    replace_relate = {
        ...
        'more closed': 'more closed to',
        'more open': 'more open to'
    }
    ...
    for state in final_states:
        ...
        for name, val in state['vals'].items():
            try:
                val = replace_relate[val]
            except:
                tmp = 0
            if val in synonyms:
                val = random.choice(synonyms[val])
            if args.only_query_label:
                if name == '<L>':
    ...
    # if the question is stupid, which means you can find the answer
    # in the question text, then skip it
    if not args.only_query_label:
        for a in answers:
    ...
    try:
        if structured_questions[0][-2]['type'] == 'same_size':
            if scene_struct['objects'][int(structured_questions[0][-2]['_output'][0])]['size'] is None:
                text_questions, structured_questions, answers = [], [], []
                # print('replaced!!!')
        if structured_questions[0][-2]['type'] == 'same_color':
            if scene_struct['objects'][int(structured_questions[0][-2]['_output'][0])]['color'] is None:
                text_questions, structured_questions, answers = [], [], []
                # print('replaced!!!')
        if structured_questions[0][-2]['type'] == 'same_material':
            if scene_struct['objects'][int(structured_questions[0][-2]['_output'][0])]['material'] is None:
                text_questions, structured_questions, answers = [], [], []
                # print('replaced!!!')
        if structured_questions[0][-2]['type'] == 'same_shape':
            if scene_struct['objects'][int(structured_questions[0][-2]['_output'][0])]['shape'] is None:
                text_questions, structured_questions, answers = [], [], []
                # print('replaced!!!')
...
def main(args):
    ...
    metadata['_functions_by_name'] = functions_by_name  # dict of dicts
    ...
    for fn in os.listdir(args.template_dir):
        ...
        with open(os.path.join(args.template_dir, fn), 'r') as f:
            ...
            for i, template in enumerate(json.load(f)):  # template: {...}
                if ('L' + str(args.num_of_objects + 1)) in template['text'][0]:
                    continue
                if args.only_relate:
                    if not '<R>' in template['text'][0]: continue
                if args.only_query:
                    is_query = False
                    if template['nodes'][-1]['type'] in ['query_color', 'query_shape', 'query_material']:
                        is_query = True
    ...
    while total_num_of_questions < args.total_num_of_questions:
        for i, scene in enumerate(tqdm(all_scenes)):
            scene_scan = scene['scan']
            scene_fn = scene['image_filename']
            scene_struct = scene
            print('starting scene %s (%d / %d)' % (scene_scan, i + 1, len(all_scenes)))
            ...
            for (fn, idx), template in templates_items:
                ...
                if args.time_dfs and args.verbose:
                    tic = time.time()
                ts, qs, ans = instantiate_templates_dfs(args,
                                                        scene_struct,
                                                        template,
                                                        metadata,
                ...
                for t, q, a in zip(ts, qs, ans):
                    ...
                    questions.append({
                        ...
                        'question_type': q[-1]['type']
                    })

                    obj_ids = []
                    for step_i in q[-1]['inputs']:
                        obj_ids.append(q[step_i]['_output'])
    ...
    with open(args.output_questions_only_file, 'w') as fonly:
        json.dump({
            'questions': questions_only,
        }, fonly, indent=4, ensure_ascii=False)
