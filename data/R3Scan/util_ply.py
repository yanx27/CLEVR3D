...
def load_rgb(path, target_name=define.LABEL_FILE_NAME):
    ...
    if pth_label != pth_label_raw:
        ...
        if 'nx' not in label_mesh_align.metadata['ply_raw']['vertex']['data']:
            ...
            label_mesh_align.metadata['ply_raw']['vertex']['data']['nx'] = np.zeros([len(query_points), 1],
                                                                                    dtype=np.float32)
            label_mesh_align.metadata['ply_raw']['vertex']['data']['ny'] = np.zeros([len(query_points), 1],
                                                                                    dtype=np.float32)
            label_mesh_align.metadata['ply_raw']['vertex']['data']['nz'] = np.zeros([len(query_points), 1],
                                                                                    dtype=np.float32)
