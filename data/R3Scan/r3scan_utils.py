...
def read_label_mapping(filename, label_from='Label', label_to='NYU40_ID'):
...
def random_point_dropout(pts, max_dropout_ratio=0.875):
    ''' pc: Nx3 '''
    dropout_ratio = np.random.random() * max_dropout_ratio  # 0~0.875
    drop_idx = np.where(np.random.random((pts.shape[0])) <= dropout_ratio)[0]
    if len(drop_idx) > 0:
        pts[drop_idx, :] = pts[0, :]  # set to the first point
    return pts