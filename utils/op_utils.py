...
def gen_descriptor_batch(pts: torch.tensor):
    '''
    centroid_pts,std_pts,segment_dims,segment_volume,segment_lengths
    [3, 3, 3, 1, 1]
    '''
    assert pts.ndim == 4, pts.ndim
    assert pts.shape[-1] == 3, pts.shape
    # centroid [b, 3]
    centroid_pts = pts.mean(2)
    # # std [b, 3]
    std_pts = pts.std(2)
    # dimensions [b, 3]
    segment_dims = pts.max(dim=2)[0] - pts.min(dim=2)[0]
    # volume [b, 1]
    segment_volume = (segment_dims[..., 0] * segment_dims[..., 1] * segment_dims[..., 2]).unsqueeze(2)
    # length [n, 1]
    segment_lengths = segment_dims.max(dim=2)[0].unsqueeze(2)
    return torch.cat([centroid_pts, std_pts, segment_dims, segment_volume, segment_lengths], dim=2)
